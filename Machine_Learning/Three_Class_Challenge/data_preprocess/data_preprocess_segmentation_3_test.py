...
def process_image_directory(directory):
    file_paths = [os.path.join(subdir, file) for subdir, _, files in os.walk(directory) for file in files]
...
if __name__ == '__main__':
    test_dir = 'test_3/test_3'  
    X_test = process_image_directory(test_dir)

    scaler = StandardScaler()
    X_test_scaled = scaler.fit_transform(X_test)

    np.save('X_test_3_segmentation.npy', X_test_scaled)
    print("Test image feature extraction and hair removal complete. Data saved to 'X_test_3_segmentation.npy'.")