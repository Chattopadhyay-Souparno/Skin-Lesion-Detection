...
def process_single_image(file_path, label, total_files):
...
def process_image_directory(directory, label):
    ...
    with ThreadPoolExecutor() as executor:
        futures = {executor.submit(process_single_image, file, label, total_files): file for file in file_paths}
...
train_mel_features, train_mel_labels = process_image_directory(train_mel_path, label=0)
train_bcc_features, train_bcc_labels = process_image_directory(train_bcc_path, label=1)
train_scc_features, train_scc_labels = process_image_directory(train_scc_path, label=2)
...
val_mel_features, val_mel_labels = process_image_directory(val_mel_path, label=0)
val_bcc_features, val_bcc_labels = process_image_directory(val_bcc_path, label=1)
val_scc_features, val_scc_labels = process_image_directory(val_scc_path, label=2)
...
X_train, X_val_train, y_train, y_val_train = train_test_split(X_train, y_train, test_size=0.2, random_state=42)


np.save('X_train_3.npy', X_train)  
np.save('y_train_3.npy', y_train)  
np.save('X_val_3.npy', X_val)      
np.save('y_val_3.npy', y_val)      

print("Feature extraction, hair removal, and data saving complete.")